        ...
        for index in range(12):
            ...
            records.append(
                ExecutionRecord(
                    ...
                    finished_at=float(index) + 1.0,
                    predicted_latency=1.0,
                    actual_latency=1.0,
                    actual_ttft=0.5,
                    estimated_reusable_tokens=raw_estimate,
                    actual_reusable_tokens=actual_reuse,
                    estimated_remaining_prefill_tokens=input_length - raw_estimate,
                    input_length=input_length,
                    continuation_tokens=4,
                    reuse_fraction=actual_reuse / input_length,
                    network_cost=0.0,
                    queue_delay=0.0,
        ...
        for index, (
            ...
            records.append(
                ExecutionRecord(
                    ...
                    arrival_time=float(index),
                    started_at=float(index),
                    finished_at=float(index) + residual_latency + 0.01,
                    predicted_latency=10.0,
                    actual_latency=0.005 + residual_latency,
                    actual_ttft=0.005 + residual_latency / 2.0,
                    estimated_reusable_tokens=64,
                    actual_reusable_tokens=64,
                    estimated_remaining_prefill_tokens=remaining_prefill,
                    input_length=64 + remaining_prefill,
                    continuation_tokens=continuation,
                    reuse_fraction=0.5,
                    network_cost=0.005,
                    queue_delay=queue_depth * target_config.queue_depth_penalty,
                    ...
                    failover_delay=0.0,
                    attempt_count=1,
                    service_time=residual_latency,
                )
            )
        calibrated_config, calibration = fit_router_config(records, base_config)
        self.assertAlmostEqual(
            calibrated_config.fixed_overhead, target_config.fixed_overhead, places=4
        )
        self.assertAlmostEqual(
            calibrated_config.prefill_cost_per_token,
            target_config.prefill_cost_per_token,
            places=4,
        )
        self.assertAlmostEqual(
            calibrated_config.decode_cost_per_token,
            target_config.decode_cost_per_token,
            places=4,
        )
        self.assertAlmostEqual(
            calibrated_config.queue_depth_penalty,
            target_config.queue_depth_penalty,
            places=4,
        )
        self.assertAlmostEqual(
            calibrated_config.stale_penalty_per_second,
            target_config.stale_penalty_per_second,
            places=4,
        )
        self.assertAlmostEqual(
            calibrated_config.uncertainty_penalty_per_token,
            target_config.uncertainty_penalty_per_token,
            places=4,
        )
        self.assertAlmostEqual(
            calibrated_config.missing_summary_penalty,
            target_config.missing_summary_penalty,
            places=4,
        )
