...
def _live_test_model_path() -> Path | None:
...
def _reserve_contiguous_ports(count: int) -> tuple[int, list[socket.socket]]:
    for _ in range(128):
...
def _build_live_requests() -> list[Request]:
    prompts = [
        "System:\nYou route inference traffic across data centers.\n\nUser:\nSummarize the queue state for tenant alpha.",
        "System:\nYou route inference traffic across data centers.\n\nUser:\nSummarize the queue state for tenant beta.",
...
class LiveLlamaCppIntegrationTests(unittest.TestCase):
    def test_benchmark_runner_exercises_validation_and_fault_injection_live(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="orbit-live-integration-") as tmpdir, mock.patch(
            "orbit.benchmark.generate_workload",
            ...
            expected_paths = [
                ...
                output_dir / "load_only_records.json",
            ]
            ...
            self.assertEqual(selection["selection_metric"], "validation_prediction_mae")
