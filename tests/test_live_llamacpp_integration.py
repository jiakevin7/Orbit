...
class LiveLlamaCppIntegrationTests(unittest.TestCase):
    def test_benchmark_runner_exercises_fault_injection_live(self):
        ...
        with (
            tempfile.TemporaryDirectory(prefix="orbit-live-integration-") as tmpdir,
            mock.patch("orbit.benchmark.generate_workload", return_value=requests),
        ):
            ...
            expected_paths = [
                ...
                output_dir / "least_loaded_records.json",
            ]
            ...
            self.assertTrue(
                all((entry["prefix_token_source"] == "llama_cpp" for entry in workload))
            )
