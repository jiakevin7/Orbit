...
class MockPolicyTests(unittest.TestCase):
    ...
    def test_round_robin_policy_cycles_clusters(self):
        ...
        request_obj = Request(request_id='req', arrival_time=0.0, router_id='router-0', prefix_tokens=(1, 2, 3), continuation_tokens=1)
        first = round_robin_policy(router, request_obj, {'cluster-a': cluster_a, 'cluster-b': cluster_b}, request_obj.arrival_time, random.Random(0))
        second = round_robin_policy(router, request_obj, {'cluster-a': cluster_a, 'cluster-b': cluster_b}, request_obj.arrival_time, random.Random(0))
        third = round_robin_policy(router, request_obj, {'cluster-a': cluster_a, 'cluster-b': cluster_b}, request_obj.arrival_time, random.Random(0))
