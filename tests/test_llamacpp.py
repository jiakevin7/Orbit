...
from orbit.llamacpp import LlamaCppClient, LlamaCppCluster, LlamaCppClusterConfig, LlamaCppResult
...
class LlamaCppClientTests(unittest.TestCase):
    ...
    def test_client_waits_for_health_and_measures_stream_ttft(self):
        ...
        responses = [_FakeResponse(status=200), _FakeResponse(lines=[b'data: {"content":"hello","stop":false}\n', b'data: {"content":"","stop":true}\n', b'data: [DONE]\n'], status=200)]
    ...
    def test_client_reads_prompt_eval_latency_from_final_timings(self):
        client = LlamaCppClient(base_url='http://127.0.0.1:8081', request_timeout=5.0)
        responses = [_FakeResponse(lines=[b'data: {"content":"hello","stop":false}\n', b'data: {"content":"","stop":true,"timings":{"prompt_ms":123.0,"predicted_ms":456.0}}\n', b'data: [DONE]\n'], status=200)]
        with mock.patch('orbit.llamacpp.request.urlopen', side_effect=responses), mock.patch('orbit.llamacpp.time.perf_counter', side_effect=(0.0, 0.2, 0.3, 0.4, 0.5)):
            result = client.complete('prompt', max_tokens=4)
    ...
    def test_client_reads_processing_and_cache_ready_from_prompt_progress(self):
        client = LlamaCppClient(base_url='http://127.0.0.1:8081', request_timeout=5.0)
        responses = [_FakeResponse(lines=[b'data: {"content":"","stop":false,"prompt_progress":{"total":16,"processed":0,"time_ms":0}}\n', b'data: {"content":"","stop":false,"prompt_progress":{"total":16,"processed":16,"time_ms":23.0}}\n', b'data: {"content":"hello","stop":false}\n', b'data: {"content":"","stop":true}\n', b'data: [DONE]\n'], status=200)]
        with mock.patch('orbit.llamacpp.request.urlopen', side_effect=responses), mock.patch('orbit.llamacpp.time.perf_counter', side_effect=(0.0, 0.01, 0.04, 0.06, 0.08, 0.09, 0.1)):
            result = client.complete('prompt', max_tokens=4)
    ...
    def test_client_can_read_tokenize_and_slots_endpoints(self):
        client = LlamaCppClient(base_url='http://127.0.0.1:8081', request_timeout=5.0)
        responses = [_FakeResponse(body=b'{"tokens":[10,20,30]}', status=200), _FakeResponse(body=b'[{"id":0,"n_ctx":2048,"is_processing":true,"id_task":17},{"id":1,"n_ctx":2048,"is_processing":false}]', status=200)]
        with mock.patch('orbit.llamacpp.request.urlopen', side_effect=responses):
    ...
    def test_cluster_prepares_requests_and_prefers_live_slot_depth(self):
        cluster = LlamaCppCluster(cluster_id='cluster-a', cluster_config=ClusterConfig(), backend_config=LlamaCppClusterConfig(model_path='/tmp/placeholder.gguf', manage_server=False, parallel=2, prompt_token_cap=256))
        cluster._started = True
        request_obj = Request(request_id='req-0', arrival_time=0.0, router_id='router-a', prefix_tokens=(1, 2, 3), prompt_prefix_text='System:\nRespond briefly.\n\nUser:\nHello there.')
        with mock.patch.object(cluster._client, 'tokenize', return_value=(101, 102, 103)):
        ...
        with mock.patch.object(cluster._client, 'slots', return_value=(mock.Mock(is_processing=True), mock.Mock(is_processing=False))):
    ...
    def test_cluster_prepare_requests_truncates_to_real_prompt_token_budget(self):
        cluster = LlamaCppCluster(cluster_id='cluster-a', cluster_config=ClusterConfig(), backend_config=LlamaCppClusterConfig(model_path='/tmp/placeholder.gguf', manage_server=False, parallel=1, ctx_size=256, prompt_token_cap=64))
        cluster._started = True
        request_obj = Request(request_id='req-0', arrival_time=0.0, router_id='router-a', prefix_tokens=(1, 2, 3), prompt_prefix_text='User:\n' + '漢' * 200 + '\n\nAssistant:', continuation_tokens=16)
    ...
    def test_cluster_makes_prefix_reusable_after_prompt_eval(self):
        cluster = LlamaCppCluster(cluster_id='cluster-a', cluster_config=ClusterConfig(), backend_config=LlamaCppClusterConfig(model_path='/tmp/placeholder.gguf', manage_server=False, parallel=1))
        cluster._started = True
        first_request = Request(request_id='req-0', arrival_time=0.0, router_id='router-a', prefix_tokens=(7, 8, 9), prompt_prefix_text='System:\nRespond briefly.\n\nUser:\nHello there.', prefix_token_source='llama_cpp')
        second_request = Request(request_id='req-1', arrival_time=0.3, router_id='router-a', prefix_tokens=(7, 8, 9), prompt_prefix_text='System:\nRespond briefly.\n\nUser:\nHello there.', prefix_token_source='llama_cpp')
        with mock.patch.object(cluster._client, 'complete', return_value=LlamaCppResult(total_latency=1.0, ttft=0.4, prompt_eval_latency=0.2, processing_started_latency=0.0, cache_ready_latency=0.2)), mock.patch.object(cluster._client, 'slots', side_effect=RuntimeError('offline')):
