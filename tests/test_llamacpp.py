...
class LlamaCppClientTests(unittest.TestCase):
    def test_client_waits_for_health_and_measures_stream_ttft(self) -> None:
    ...
    def test_client_reads_prompt_eval_latency_from_final_timings(self) -> None:
        client = LlamaCppClient(base_url="http://127.0.0.1:8081", request_timeout=5.0)
        responses = [
            _FakeResponse(
                lines=[
                    b'data: {"content":"hello","stop":false}\n',
                    (
                        b'data: {"content":"","stop":true,"timings":{"prompt_ms":123.0,'
                        b'"predicted_ms":456.0}}\n'
                    ),
                    b"data: [DONE]\n",
                ],
                status=200,
        ...
        with mock.patch("orbit.llamacpp.request.urlopen", side_effect=responses), mock.patch(
    ...
    def test_client_reads_processing_and_cache_ready_from_prompt_progress(self) -> None:
        client = LlamaCppClient(base_url="http://127.0.0.1:8081", request_timeout=5.0)
        responses = [
            _FakeResponse(
                lines=[
                    (
                        b'data: {"content":"","stop":false,"prompt_progress":{"total":16,'
                        b'"processed":0,"time_ms":0}}\n'
                    ),
                    (
                        b'data: {"content":"","stop":false,"prompt_progress":{"total":16,'
                        b'"processed":16,"time_ms":23.0}}\n'
                    ),
                    b'data: {"content":"hello","stop":false}\n',
                    b'data: {"content":"","stop":true}\n',
                    b"data: [DONE]\n",
    ...
    def test_client_can_read_tokenize_and_slots_endpoints(self) -> None:
        client = LlamaCppClient(base_url="http://127.0.0.1:8081", request_timeout=5.0)
        responses = [
            _FakeResponse(body=b'{"tokens":[10,20,30]}', status=200),
            _FakeResponse(
                body=(
                    b'[{"id":0,"n_ctx":2048,"is_processing":true,"id_task":17},'
                    b'{"id":1,"n_ctx":2048,"is_processing":false}]'
                ),
                status=200,
            ),
        ]
    ...
    def test_cluster_prepares_requests_and_prefers_live_slot_depth(self) -> None:
