...
class ArtifactTests(unittest.TestCase):
    def test_reporting_csv_json_and_grouped_summaries(self):
        ...
        metrics_by_policy = {
            "orbit": SimulationMetrics(
                policy="orbit",
                request_count=2,
                mean_reusable_prefix=4.0,
                mean_reuse_fraction=0.5,
                ttft_p50=0.75,
                ttft_p95=1.0,
                latency_p50=1.5,
                latency_p95=2.0,
                ...
                failover_count=1,
                failover_rate=0.5,
                cluster_request_counts={"cluster-0": 1, "cluster-1": 1},
            )
        }
        ...
        grouped = summarize_execution_records(
            records, "orbit", group_field="traffic_class"
        )
    ...
    def test_png_generation_for_run_directory(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            (run_dir / "manifest.json").write_text(
                json.dumps({"backend": "synthetic", "policies": ["orbit", "random"]}),
                encoding="utf-8",
            )
            ...
            (run_dir / "orbit_records.json").write_text(
                json.dumps(
                    [
                        {
                            "request_id": "orbit-0",
