...
class SimulationTests(unittest.TestCase):
    def test_simulation_runs_and_returns_metrics(self) -> None:
    ...
    def test_oracle_is_not_worse_than_random_on_reuse(self) -> None:
        ...
        self.assertGreaterEqual(
            metrics["oracle"].mean_reusable_prefix,
            metrics["random"].mean_reusable_prefix,
        )

    def test_llama_cpp_defaults_use_second_scaled_network_costs(self) -> None:
        config = SimulationConfig(
            backend="llama_cpp",
            router_ids=("router-a", "router-b"),
            cluster_ids=("cluster-a", "cluster-b"),
    ...
    def test_llama_cpp_prepare_requests_is_idempotent_for_already_aligned_workloads(self) -> None:
        requests = [
            Request(
                request_id="req-0",
                arrival_time=0.0,
    ...
    def test_llama_cpp_prepare_requests_scales_arrivals_once(self) -> None:
        simulation = Simulation(
            SimulationConfig(
                backend="llama_cpp",
                router_ids=("router-a",),
    ...
    def test_synthetic_cluster_exposes_prefix_reuse_after_prefill_not_finish(self) -> None:
        cluster = Cluster(
            cluster_id="cluster-a",
            config=ClusterConfig(
                concurrency=1,
                prefill_cost_per_token=1.0,
                decode_cost_per_token=10.0,
            ),
        )
    ...
    def test_llama_cpp_run_replays_requests_concurrently(self) -> None:
        config = SimulationConfig(
            backend="llama_cpp",
            router_ids=("router-a",),
            cluster_ids=("cluster-a",),
        ...
        def fake_complete(prompt: str, max_tokens: int, event_callback=None) -> LlamaCppResult:
            ...
            with gate:
                ...
                if event_callback is not None:
                    event_callback(
                        {"prompt_progress": {"total": 3, "processed": 0, "time_ms": 0.0}},
                        processing_started,
                    )
                time.sleep(0.01)
                ...
                if event_callback is not None:
                    event_callback(
                        {"prompt_progress": {"total": 3, "processed": 3, "time_ms": 10.0}},
                        cache_ready,
                    )
                time.sleep(0.03)
        ...
        try:
            with mock.patch.object(cluster, "_ensure_started"), mock.patch.object(
                cluster._client,
                "complete",
                side_effect=fake_complete,
            ), mock.patch.object(cluster._client, "slots", side_effect=RuntimeError("offline")):
                records, _ = simulation.run("summary", requests=requests)
        finally:
            simulation.close()
    ...
    def test_cluster_outage_records_failover(self) -> None:
        config = SimulationConfig(
            router_ids=("router-a",),
            cluster_ids=("cluster-a", "cluster-b"),
            cluster_config=ClusterConfig(
