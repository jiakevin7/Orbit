...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Sweep Orbit recent-assignment pressure knobs on the large mixed_external setup",
    )
    parser.add_argument(
        "--model",
    ...
    parser.add_argument("--llama-port-base", type=int, default=27010)
    ...
    parser.add_argument(
        ...
        type=float,
        default=[0.5, 1.0, 2.0],
        help="weight added per recent assignment to the effective queue depth",
    )
...
def build_output_dir(path: str | None) -> Path:
    ...
    return (REPO_ROOT / "results" / f"orbit-recent-pressure-sweep-{timestamp}").resolve()


def candidate_name(window: float, weight: float) -> str:
    if window <= 0:
        return "baseline"
    return f"win{window:.2f}_wt{weight:.2f}".replace(".", "p")
...
def main(argv: list[str] | None = None) -> int:
    ...
    candidate_grid = [(0.0, 0.0)] + list(
        itertools.product(args.window_values, args.weight_values)
    )
    ...
    write_json(
        output_dir / "manifest.json",
        {
            ...
            "weight_values": list(args.weight_values),
    ...
    for candidate_index, (window, weight) in enumerate(candidate_grid):
        name = candidate_name(window, weight)
        ...
        config_payload = {
            ...
            "recent_assignment_weight": weight,
        }
        ...
        write_json(candidate_dir / "aggregate.json", aggregate)
