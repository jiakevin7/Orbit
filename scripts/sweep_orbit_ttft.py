...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Sweep Orbit TTFT router coefficients on the mixed_external workload",
    )
    parser.add_argument(
        "--model",
    ...
    parser.add_argument("--llama-port-base", type=int, default=25010)
    ...
    parser.add_argument("--measured-requests", type=int, default=24)
    ...
    parser.add_argument("--seeds", nargs="+", type=int, default=[7, 11, 17])
    parser.add_argument(
        "--prefill-values",
        nargs="+",
        type=float,
    ...
    parser.add_argument("--fixed-overhead", type=float, default=0.0)
    parser.add_argument("--missing-summary-penalty", type=float, default=0.0)
...
def build_output_dir(path: str | None) -> Path:
    ...
    return (REPO_ROOT / "results" / f"orbit-ttft-sweep-{timestamp}").resolve()


def candidate_name(prefill: float, load: float, queue: float) -> str:
    return (
        f"p{prefill:.2f}".replace(".", "p")
        + f"_l{load:.2f}".replace(".", "p")
        + f"_q{queue:.2f}".replace(".", "p")
    )
...
def main(argv: list[str] | None = None) -> int:
    ...
    total_requests = args.measured_requests + args.warmup_requests

    candidate_grid = [
        (prefill, load, queue)
        for prefill, load, queue in itertools.product(
            args.prefill_values,
            args.load_values,
            args.queue_values,
        )
    ...
    write_json(
        output_dir / "manifest.json",
        {
            ...
            "missing_summary_penalty": args.missing_summary_penalty,
            "prefill_values": list(args.prefill_values),
            "load_values": list(args.load_values),
            "queue_values": list(args.queue_values),
    ...
    for candidate_index, (prefill, load, queue) in enumerate(candidate_grid):
        name = candidate_name(prefill, load, queue)
        print(f"candidate {name} ({candidate_index + 1}/{len(candidate_grid)})", flush=True)
        candidate_dir = output_dir / name
        candidate_dir.mkdir(parents=True, exist_ok=True)
        config_payload = {
            "candidate": name,
            "ttft_fixed_overhead": args.fixed_overhead,
            "ttft_prefill_cost_per_token": prefill,
            "ttft_load_penalty": load,
            "ttft_queue_depth_penalty": queue,
            "ttft_missing_summary_penalty": args.missing_summary_penalty,
            "fixed_overhead": args.fixed_overhead,
            "prefill_cost_per_token": prefill,
            "load_penalty": load,
            "queue_depth_penalty": queue,
            "missing_summary_penalty": args.missing_summary_penalty,
        }
    ...
    print(f"best={best['candidate']} ttft_p50={best['ttft_p50_mean']:.3f} ttft_p95={best['ttft_p95_mean']:.3f}", flush=True)
