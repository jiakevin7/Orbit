...
# Traffic classes in the final mixed workload. Bursty traffic can still be
# produced as follow-ups within each class, but standalone bursty is disabled.
...
def load_sharegpt_conversations(path, sample_limit=2000):
    # Accept common ShareGPT-style shapes and deduplicate conversations so one
    # repeated export does not dominate prefix locality.
    if path is None:
        return []
    ...
    for index, record in enumerate(records):
        ...
        signature = hashlib.blake2b(
            repr(example.messages).encode("utf-8"), digest_size=16
        ).hexdigest()
...
def load_rag_examples(path, sample_limit=2000):
    # RAG loaders normalize many public dataset field names into query plus
    # retrieved contexts, which is all the workload generator needs.
    ...
    for index, record in enumerate(records):
        ...
        if example is None:
            continue
        signature = hashlib.blake2b(
            repr((example.query, example.contexts)).encode("utf-8"), digest_size=16
        ).hexdigest()
        if signature in seen_signatures:
            continue
        seen_signatures.add(signature)
...
def load_agent_conversations(path, sample_limit=2000):
    # Agent traffic supports tool/dialogue datasets such as BFCL and tau-bench,
    # but falls back to generic tool-use examples when no file is provided.
...
def render_prompt_from_words(words):
    ...
    context_words = words[
        system_count + tool_count : system_count + tool_count + context_count
    ]
...
def _generate_synthetic_workload(config):
    ...
    weights = [
        1.0 / (index + 1) ** config.template_popularity_alpha
        for index in range(config.num_templates)
    ]
...
def _generate_mixed_realistic_workload(config):
    # Mix chat, RAG, and agent requests while preserving session affinity and
    # repeated source variants that create realistic prefix reuse opportunities.
    rng = random.Random(config.seed)

    # Load external datasets when present. Each loader returns a normalized
    # internal shape so the rest of generation does not depend on dataset schema.
    chat_examples = load_sharegpt_conversations(
        config.sharegpt_path, sample_limit=config.sharegpt_sample_limit
    )
    rag_examples = load_rag_examples(
        config.rag_path, sample_limit=config.rag_sample_limit
    )
    agent_examples = load_agent_conversations(
        config.agent_path, sample_limit=config.agent_sample_limit
    )

    # Synthetic fallback examples keep tests and demos runnable without copying
    # external datasets into the repository.
    ...
    # Session affinity and source variants create realistic repeated prefixes
    # without cloning identical requests.
    ...
    # Force at least one request from every positive traffic class before using
    # weighted random sampling for the remaining requests.
    ...
    if not positive_traffic_types:
        raise ValueError(
            "mixed_realistic workload requires at least one positive traffic weight"
        )
    ...
    while len(requests) < config.num_requests:
        ...
        reserve_for_required = (
            len(positive_traffic_types) - 1 if positive_traffic_types else 0
        )
        ...
        # Each branch creates one primary request, then optionally appends a
        # small same-session follow-up burst that preserves most of the prefix.
    ...
    return sorted(
        ...
        key=lambda request_obj: request_obj.arrival_time,
    )
...
def _build_sharegpt_chat_request(
    ...
    assistant_response = (
        _next_message_content(example.messages, user_index + 1, "assistant") or ""
    )
    ...
    if rag_examples:
        ...
        prompt_sections = [
            ...
            (
                "Retrieved Context",
                "\n\n".join(
                    (f"[{doc_id}]\n{doc_text}" for doc_id, doc_text in chosen_docs)
                ),
            ),
            ("User", example.query),
            ("Assistant", ""),
        ...
        continuation_seed = example.answer or " ".join(
            (doc_text for _, doc_text in chosen_docs[:1])
        )
        return _build_request(
            config=config,
            request_id=f"req-{request_index:05d}",
            ...
            router_id=rng.choice(config.router_ids),
            prompt_prefix_text=prompt_prefix_text,
            continuation_tokens=_continuation_tokens_from_text(
                continuation_seed, config
            ),
            traffic_class=RAG_TRAFFIC_CLASS,
            source_id=source_id,
        )
...
def _build_agent_request(
    ...
    if agent_examples:
        example, user_index = _next_conversation_turn(
            agent_turn_pool, agent_examples, rng, agent_mode=True
        )
        prompt_prefix_text = render_message_prompt(example.messages[: user_index + 1])
        continuation_seed = (
            _next_agent_turn_seed(example.messages, user_index + 1)
            or example.messages[user_index][1]
        )
    ...
    prompt_sections = [
        ...
        (
            "Tools",
            "\n".join(
                (f"- {tool_name}: {schema_text}" for tool_name, schema_text in catalog)
            ),
        ),
...
def _build_bursty_requests(
    ...
    assistant_indices = [
        index for index, (role, _) in enumerate(example.messages) if role == "assistant"
    ]
    ...
    previous_continuation = max(
        config.dataset_continuation_floor, config.continuation_token_range[1]
    )
    for offset in range(burst_size):
        if previous_prompt is not None and rng.random() < 0.3:
            ...
            continuation_tokens = max(
                ...
                min(
                    config.dataset_continuation_cap, len(_lexical_tokens(followup)) * 3
                ),
            )
        ...
        current_arrival += max(
            ...
            * config.burst_interarrival_ratio
...
def _build_followup_burst_requests(
    *, base_request, config, rng, request_index_start, max_requests
):
    # Follow-up bursts reuse most of the prior prompt with a small appended turn;
    # this models conversational bursts without a separate standalone dataset.
    ...
    for offset in range(1, burst_size):
        ...
        followup_prompt = _build_followup_prompt_from_request(
            base_request=base_request, rng=rng, offset=offset
        )
...
def _build_request(
    ...
    normalized_prompt_prefix_text = _truncate_prompt_prefix_text(
        prompt_prefix_text, config.prompt_prefix_token_cap
    )
    return Request(
...
def _normalize_rag_record(record, fallback_index):
    ...
    query = _first_text_value(
        record, ("query", "question", "input", "prompt", "instruction")
    )
    if query is None:
        return None
    ...
    return RagExample(
        example_id=example_id, query=query, contexts=tuple(contexts), answer=answer
    )


def _normalize_agent_record(record, fallback_index):
    # Agent datasets vary widely, so try known benchmark schemas first and then
    # fall back to generic tool metadata plus user/assistant turns.
    if not isinstance(record, dict):
        return None
    ...
    user_prompt = _first_text_value(
        record, ("question", "query", "instruction", "user", "prompt")
    )
    ...
    messages = [
        (
            "system",
            "You are a tool-using assistant. Choose safe tool calls before answering the user.",
        )
    ]
...
def _normalize_bfcl_record(record, fallback_index):
    ...
    if not messages:
        question_text = _first_text_value(
            record, ("question", "query", "instruction", "prompt")
        )
    ...
    if answer_text is not None and (
        not any((role == "assistant" for role, _ in normalized_messages))
    ):
        normalized_messages.append(("assistant", answer_text))
    if not any((role == "user" for role, _ in normalized_messages)):
        return None
    conversation_id = str(
        record.get("id") or record.get("question_id") or f"bfcl-{fallback_index}"
    )
    return ConversationExample(
        conversation_id=conversation_id, messages=tuple(normalized_messages)
    )
...
def _normalize_tau_bench_record(record, fallback_index):
    if not any(
        (
            key in record
            for key in ("task", "trajectory", "history", "trace", "env", "domain")
        )
    ):
        return None
    ...
    if isinstance(task, dict):
        ...
        goal = _first_text_value(
            task, ("instruction", "user_instruction", "goal", "description", "task")
        )
    ...
    for field_name in (
        ...
        if extracted:
            ...
            break
    if not any((role == "user" for role, _ in messages)):
        prompt_text = _first_text_value(
            record, ("instruction", "query", "prompt", "goal", "description")
        )
...
def _build_chat_turn_pool(chat_examples, rng):
    pool = [
        ...
        if role == "user"
        and _next_message_content(example.messages, index + 1, "assistant") is not None
    ]
    rng.shuffle(pool)
    return pool
...
def _choose_router_for_session(
    router_ids, session_id, session_router_map, affinity_probability, rng
):
...
def _apply_request_variant(
    *, prompt_prefix_text, traffic_class, base_source_id, source_variant_counts
):
...
def _build_followup_prompt_from_text(*, base_prompt_text, traffic_class, offset):
    followup_catalog = _FOLLOWUP_PROMPTS_BY_TRAFFIC.get(
        traffic_class, _FOLLOWUP_PROMPTS_BY_TRAFFIC[CHAT_TRAFFIC_CLASS]
    )
    ...
    if base_prompt.endswith("Assistant:"):
        assistant_bridge = _ASSISTANT_BRIDGE_BY_TRAFFIC.get(
            traffic_class, _ASSISTANT_BRIDGE_BY_TRAFFIC[CHAT_TRAFFIC_CLASS]
        )
    ...
    return [
        ...
        ConversationExample(
            ...
            messages=(
                ...
                ("user", "Write the reply in a calm tone with one concrete next step."),
            ),
        ),
    ]
    ...
    while len(suffix_words) < length:
        suffix_words.extend(
            [
                ...
                rng.choice(
                    ("incident", "request", "workflow", "handoff", "ticket", "report")
                ),
