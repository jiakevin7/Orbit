...
def summarize_execution_records(records, policy_name, group_field=None):
    # Grouped summaries power traffic/source breakdowns without another
    ...
    for group_value, group_records in sorted(
        grouped.items(), key=lambda item: str(item[0]) if item[0] is not None else ""
    ):
        ...
        row = {
            ...
            "failover_rate": failover_count / len(group_records)
            if group_records
            else 0.0,
        }
