...
def execution_records_as_dicts(records):
    return [asdict(record) for record in records]

def requests_as_dicts(requests):
    return [asdict(request_obj) for request_obj in requests]

def metrics_as_dict(metrics):
    return asdict(metrics)

def metrics_rows_by_policy(metrics_by_policy):
    cluster_ids = sorted({cluster_id for metrics in metrics_by_policy.values() for cluster_id in metrics.cluster_request_counts})
...
def write_execution_records_csv(path, records):
    destination = Path(path)
    destination.parent.mkdir(parents=True, exist_ok=True)
...
def summarize_execution_records(records, policy_name, group_field=None):
    ...
    for group_value, group_records in sorted(grouped.items(), key=lambda item: str(item[0]) if item[0] is not None else ''):
        ...
        row = {'policy': policy_name, 'request_count': len(group_records), 'mean_reusable_prefix': statistics.fmean(reusable_prefixes) if reusable_prefixes else 0.0, 'mean_reuse_fraction': statistics.fmean(reuse_fractions) if reuse_fractions else 0.0, 'ttft_p50': _percentile(ttfts, 0.5), 'ttft_p95': _percentile(ttfts, 0.95), 'latency_p50': _percentile(latencies, 0.5), 'latency_p95': _percentile(latencies, 0.95), 'failover_count': failover_count, 'failover_rate': failover_count / len(group_records) if group_records else 0.0}
