...
class Cluster:
    def __init__(self, cluster_id: str, config: ClusterConfig | None = None) -> None:
    ...
    def execute(self, request: Request) -> ClusterExecution:
        ...
        heapq.heappush(
            ...
            (cache_ready_at, self._cache_sequence, request.request_id, request.prefix_tokens),
        )
    ...
    def publish_summary(self, now: float) -> ClusterSummary:
        ...
        filters = {
            depth: BloomFilter(
                num_bits=self.config.bloom_bits,
