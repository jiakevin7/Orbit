...
POLICY_ORDER = ('orbit', 'least_loaded', 'random', 'round_robin', 'summary', 'load_only', 'exact_prefix', 'oracle')
POLICY_LABELS = {'orbit': 'Orbit', 'summary': 'Orbit', 'least_loaded': 'Least Loaded', 'load_only': 'Least Loaded', 'round_robin': 'Round Robin', 'random': 'Random', 'exact_prefix': 'Exact Prefix', 'oracle': 'Oracle'}
POLICY_PALETTE = {'orbit': '#1d4ed8', 'summary': '#1d4ed8', 'least_loaded': '#ea580c', 'round_robin': '#2563eb', 'load_only': '#ea580c', 'random': '#b91c1c', 'exact_prefix': '#047857', 'oracle': '#6d28d9'}
POLICY_LINESTYLES = {'orbit': '-', 'summary': '-', 'least_loaded': '-.', 'round_robin': (0, (2, 2)), 'load_only': '-.', 'random': ':', 'exact_prefix': '--', 'oracle': (0, (5, 2))}
...
def _generate_single_run_plots(run_dir):
    ...
    sns.set_theme(style='whitegrid', context='talk', rc={'axes.spines.top': False, 'axes.spines.right': False, 'axes.titleweight': 'semibold', 'legend.frameon': False})
    ...
    created.append(_save_ecdf_plot(dataframe, x_field='actual_ttft', output_path=plots_dir / 'ttft_cdf.png', title='TTFT ECDF by Routing Policy', x_label='TTFT (s)', subtitle='Pooled measured requests across seeds' if aggregate_mode else None))
    created.append(_save_ecdf_plot(dataframe, x_field='actual_latency', output_path=plots_dir / 'latency_cdf.png', title='End-to-End Latency ECDF by Routing Policy', x_label='Latency (s)', subtitle='Pooled measured requests across seeds' if aggregate_mode else None))
    created.append(_save_policy_interval_plot(dataframe, summary_runs, value_field='actual_ttft', p50_field='ttft_p50', p95_field='ttft_p95', output_path=plots_dir / 'ttft_by_policy.png', title='TTFT by Policy', x_label='TTFT (s)'))
    created.append(_save_policy_interval_plot(dataframe, summary_runs, value_field='actual_latency', p50_field='latency_p50', p95_field='latency_p95', output_path=plots_dir / 'latency_by_policy.png', title='Latency by Policy', x_label='Latency (s)'))
    created.append(_save_tradeoff_plot(dataframe, summary_runs, output_path=plots_dir / 'reuse_latency_tradeoff.png', title='Reuse vs Median Latency'))
    if traffic_rows:
        created.append(_save_traffic_point_plot(traffic_rows, metric='latency_p50', output_path=plots_dir / 'latency_by_traffic.png', title='Median Latency by Traffic Class', y_label='Latency p50 (s)'))
        created.append(_save_traffic_point_plot(traffic_rows, metric='mean_reusable_prefix', output_path=plots_dir / 'reuse_by_traffic.png', title='Reusable Prefix by Traffic Class', y_label='Mean reusable prefix (tokens)'))
    plt.close('all')
...
def _generate_summary_only_plots(run_dir, summary_runs, traffic_rows):
    ...
    _clear_existing_plots(plots_dir)
    created: list[Path] = []
    created.append(_save_policy_interval_plot(None, summary_runs, value_field='actual_ttft', p50_field='ttft_p50', p95_field='ttft_p95', output_path=plots_dir / 'ttft_by_policy.png', title='TTFT by Policy', x_label='TTFT (s)'))
    created.append(_save_policy_interval_plot(None, summary_runs, value_field='actual_latency', p50_field='latency_p50', p95_field='latency_p95', output_path=plots_dir / 'latency_by_policy.png', title='Latency by Policy', x_label='Latency (s)'))
    created.append(_save_tradeoff_plot(summary_frame, summary_runs, output_path=plots_dir / 'reuse_latency_tradeoff.png', title='Reuse vs Median Latency'))
    if traffic_rows:
        created.append(_save_traffic_point_plot(traffic_rows, metric='latency_p50', output_path=plots_dir / 'latency_by_traffic.png', title='Median Latency by Traffic Class', y_label='Latency p50 (s)'))
        created.append(_save_traffic_point_plot(traffic_rows, metric='mean_reusable_prefix', output_path=plots_dir / 'reuse_by_traffic.png', title='Reusable Prefix by Traffic Class', y_label='Mean reusable prefix (tokens)'))
    plt.close('all')
...
def _prepare_dataframe(dataframe):
    ...
    for column in ('actual_ttft', 'actual_latency', 'predicted_latency', 'reuse_fraction', 'failover_delay', 'route_queue_depth'):
...
def _save_ecdf_plot(dataframe, x_field, output_path, title, x_label, subtitle=None):
    ...
    handles = [Line2D([0], [0], color=palette[policy], linestyle=POLICY_LINESTYLES.get(policy, '-'), linewidth=3.2 if policy == 'summary' else 2.5) for policy in order]
    labels = [f"{_display_name(policy)}  p50={stats[policy]['p50']:.3f}s  p95={stats[policy]['p95']:.3f}s" for policy in order]
...
def _save_policy_interval_plot(dataframe, summary_runs, value_field, p50_field, p95_field, output_path, title, x_label):
    import matplotlib.pyplot as plt
    from matplotlib.lines import Line2D
    import pandas as pd
    if summary_runs:
        ...
        stats = {policy: {'p50_mean': values['p50'], 'p95_mean': values['p95'], 'p50_values': [values['p50']], 'p95_values': [values['p95']]} for policy, values in stats.items()}
...
def _save_tradeoff_plot(dataframe, summary_runs, output_path, title):
    ...
    if summary_runs:
        ...
        reuse_field = next((field for field in ('true_reusable_prefix', 'actual_reusable_tokens', 'estimated_reusable_tokens', 'reuse_fraction') if field in dataframe.columns), None)
        if reuse_field is None:
            return output_path
        summary_frame = dataframe.groupby('policy', observed=False).agg(reuse_metric=(reuse_field, 'mean'), latency_metric=('actual_latency', 'median')).reset_index()
    ...
    if 'seed' in summary_frame.columns:
        sns.scatterplot(data=summary_frame, x='reuse_metric', y='latency_metric', hue='policy', hue_order=order, palette=palette, s=60, alpha=0.35, legend=False, ax=axis)
        means = summary_frame.groupby('policy', observed=False)[['reuse_metric', 'latency_metric']].mean().reset_index()
    else:
        means = summary_frame
    ...
    for _, row in means.iterrows():
        axis.annotate(_display_name(str(row['policy'])), (float(row['reuse_metric']), float(row['latency_metric'])), textcoords='offset points', xytext=(6, 6), fontsize=10)
    axis.set_title(title)
...
def _save_traffic_point_plot(traffic_rows, metric, output_path, title, y_label):
    ...
    traffic_order = [name for name in ('sharegpt_chat', 'rag', 'agent', 'bursty') if name in set(frame['traffic_class'].astype(str))]
    traffic_order.extend(sorted((name for name in frame['traffic_class'].astype(str).unique() if name not in set(traffic_order))))
    ...
    sns.lineplot(data=frame, x='traffic_class', y=metric, hue='policy', hue_order=order, style='policy', style_order=order, markers=True, dashes=True, sort=False, palette=_policy_palette(order), ax=axis)
    axis.set_title(title)
...
def _policy_quantiles(dataframe, value_field, order):
    ...
    for policy in order:
        ...
        if subset.empty:
            ...
            continue
        quantiles[policy] = {'p50': float(subset.quantile(0.5)), 'p95': float(subset.quantile(0.95))}
    return quantiles
...
def _summary_interval_stats(summary_frame, order, p50_field, p95_field):
    ...
    for policy in order:
        ...
        stats[policy] = {'p50_mean': float(sum(p50_values) / len(p50_values)) if p50_values else 0.0, 'p95_mean': float(sum(p95_values) / len(p95_values)) if p95_values else 0.0, 'p50_values': p50_values, 'p95_values': p95_values}
    return stats
