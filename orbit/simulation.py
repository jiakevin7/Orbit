...
from .process_plane import (
    ProcessClusterProxy,
    ProcessLlamaCppClusterProxy,
    ProcessRouterProxy,
)
...
def default_network_costs(
    *,
    ...
    # Sparse overlap makes the experiment closer to a real deployment: routers
    # can only reach a subset of clusters, with increasing RTT by hop distance.
    if not router_ids:
    ...
    for router_index, router_id in enumerate(router_ids):
        start = (
            int(math.floor(router_index * cluster_count / router_count)) % cluster_count
        )
        router_costs = {}
...
class Simulation:
    def __init__(self, config=None):
        ...
        # Control-plane state is event-driven so summary freshness, gossip, and
        # injected delays affect router decisions before each request arrives.
        self._next_summary_due = {
            cluster_id: 0.0 for cluster_id in self.config.cluster_ids
        }
    ...
    def run(self, policy_name="orbit", requests=(), close_on_finish=True):
        if policy_name not in POLICIES:
            raise ValueError(f"unknown policy: {policy_name}")

        workload = (
            list(requests) if requests else generate_workload(self.config.workload)
        )
        ...
        # Synthetic mode runs each request in timestamp order. Before routing a
        # request, advance summaries and cache insertions to that logical time.
        ...
        try:
            for request in workload:
                ...
                actual_latency = network_cost + (
                    execution.finished_at - request.arrival_time
                )
                ...
                if request.input_length > 0:
                    reuse_fraction = (
                        execution.true_reusable_tokens / request.input_length
                    )
                ...
                records.append(
                    ExecutionRecord(
                        ...
                        estimated_remaining_prefill_tokens=max(
                            0, request.input_length - decision.estimated_reusable_tokens
                        ),
                        ...
                        route_queue_depth=int(
                            decision.details.get("raw_queue_depth", 0.0)
                        ),
                        metadata_age=float(decision.details.get("metadata_age", 0.0)),
                        uncertainty_gap=int(
                            decision.details.get("uncertainty_gap", 0.0)
                        ),
                        missing_summary=bool(
                            decision.details.get("missing_summary", 0.0)
                        ),
                        ...
                        predicted_ttft=float(
                            decision.details.get("predicted_ttft", 0.0)
                        ),
                        predicted_route_cost=float(
                            decision.details.get(
                                "predicted_route_cost", decision.predicted_latency
                            )
                        ),
                    )
                )
            ...
            metrics = self._compute_metrics(
                ...
                records,
                cluster_counts,
                control_plane_bytes=self.control_plane_bytes
                - control_plane_bytes_start,
            )
        ...
        if self.config.control_plane_mode == "multiprocess":
            if self.config.backend == "llama_cpp":
                if self.config.llama_cpp is None:
                    raise ValueError(
                        "llama_cpp backend requires llama_cpp configuration"
                    )
                clusters = {}
    ...
    def _prepare_requests_for_backend(self, requests):
        # The live backend must route on real llama.cpp token ids, then scale
        # arrivals into wall-clock time for practical local execution.
        prepared = list(requests)
        if self.config.backend != "llama_cpp" or not prepared:
            return prepared
        if not all(
            (request_obj.prefix_token_source == "llama_cpp" for request_obj in prepared)
        ):
            ...
            if not callable(prepare_requests):
                raise RuntimeError(
                    "llama_cpp backend requires clusters that can prepare requests"
                )
        ...
        for request_obj in prepared:
            if request_obj.arrival_scale_applied == self.config.live_arrival_scale:
                ...
                continue
            logical_arrival = (
                request_obj.arrival_time / request_obj.arrival_scale_applied
            )
    ...
    def _build_home_router_map(self):
        ...
        for cluster_id, router_id in mapping.items():
            if cluster_id not in self.config.cluster_ids:
                raise ValueError(
                    f"unknown cluster in home_router_for_cluster: {cluster_id}"
                )
            if router_id not in self.config.router_ids:
                raise ValueError(
                    f"unknown router in home_router_for_cluster: {router_id}"
                )
    ...
    def _validate_topology(self):
        ...
        if unknown_routers:
            raise ValueError(
                f"network_costs contains unknown routers: {', '.join(unknown_routers)}"
            )
        for router_id in self.config.router_ids:
        ...
        for cluster_id in self.config.cluster_ids:
            if not any(
                (
                    cluster_id in self.network_costs[router_id]
                    for router_id in self.config.router_ids
                )
            ):
                raise ValueError(
                    f"cluster {cluster_id} is unreachable from every router"
                )
    ...
    def _reachable_clusters(
        self, router_id, now, *, exclude=None, only_available=False
    ):
    ...
    def _process_control_plane_until(self, now):
        # Advance summaries, gossip, and delayed deliveries in timestamp order
        # until the data plane reaches the next request time.
        while True:
            next_summary_time = min(self._next_summary_due.values())
            next_delivery_time = (
                self._pending_deliveries[0][0]
                if self._pending_deliveries
                else float("inf")
            )
            next_event_time = min(
                next_summary_time, self._next_gossip_at, next_delivery_time
            )
    ...
    def _run_llama_cpp_concurrent(self, policy_name, workload, close_on_finish):
        # Submit live requests at their scaled arrivals and collect completions
        # asynchronously, while the control plane keeps advancing in real time.
        cluster_counts: Counter[str] = Counter()
        control_plane_bytes_start = self.control_plane_bytes
        pending_by_future: dict[
            Future[object], tuple[int, Request, object, object]
        ] = {}
        ...
        try:
            ...
            # Submit requests at their scheduled scaled arrival times. Routing
            # still observes control-plane state at that moment.
            for index, request_obj in enumerate(workload):
                ...
                routed_request = (
                    request_obj
                    if abs(arrival_time - request_obj.arrival_time) < 1e-09
                ...
                if not callable(submit):
                    raise RuntimeError(
                        "llama_cpp backend requires clusters that support submit()"
                    )
            ...
            # Drain completions while continuing to deliver control-plane events.
            # This preserves summary freshness during long live requests.
            while pending_by_future:
                ...
                if not completed:
                    ...
                    if (
                        next_control_plane_event is not None
                        and next_control_plane_event > now
                    ):
                        timeout = min(
                            timeout, max(0.001, next_control_plane_event - now)
                        )
                    wait(
    ...
    def _build_execution_record(
        ...
        network_cost = router.network_cost(decision.cluster_id)
        actual_latency = network_cost + (
            execution.finished_at - request_obj.arrival_time
        )
        actual_ttft = network_cost + execution.time_to_first_token
        reuse_fraction = 0.0
        ...
        return ExecutionRecord(
            ...
            actual_reusable_tokens=execution.true_reusable_tokens,
            estimated_remaining_prefill_tokens=max(
                0, request_obj.input_length - decision.estimated_reusable_tokens
            ),
            ...
            predicted_route_cost=float(
                decision.details.get("predicted_route_cost", decision.predicted_latency)
            ),
            raw_estimated_reusable_tokens=int(
                decision.details.get(
                    "raw_estimated_reusable_tokens", decision.estimated_reusable_tokens
                )
            ),
            summary_matched_levels=int(
                decision.details.get("summary_matched_levels", 0.0)
            ),
            hotset_matched_levels=int(
                decision.details.get("hotset_matched_levels", 0.0)
            ),
        )
    ...
    def _summary_interval(self):
        if self.config.backend == "llama_cpp":
            return (
                self.config.cluster_config.summary_interval
                * self.config.live_arrival_scale
            )
    ...
    def _compute_metrics(
        self, policy_name, records, cluster_counts, control_plane_bytes
    ):
        ...
        count_values = [
            cluster_counts.get(cluster_id, 0) for cluster_id in self.config.cluster_ids
        ]
        ...
        for router in self.routers.values():
            if hasattr(router, "summary_memory_bytes"):
                ...
                summary_memory_bytes += sum(
                    (view.summary.byte_size for view in router.views.values())
                )
        return SimulationMetrics(
            policy=policy_name,
            request_count=len(records),
            mean_reusable_prefix=statistics.fmean(reusable_prefixes)
            if reusable_prefixes
            else 0.0,
            mean_reuse_fraction=statistics.fmean(reuse_fractions)
            if reuse_fractions
            else 0.0,
            ...
            load_stddev=statistics.pstdev(count_values)
            if len(count_values) > 1
            else 0.0,
            failover_count=failover_count,
            failover_rate=failover_count / len(records) if records else 0.0,
            cluster_request_counts={
                cluster_id: cluster_counts.get(cluster_id, 0)
                for cluster_id in self.config.cluster_ids
            },
        )

    def _route_request(self, policy_name, router, request, now):
        # Fault injection happens after the policy decision, so records capture
        # both the initial choice and any failover reroute.
        ...
        decision = POLICIES[policy_name](
            router, request, reachable_clusters, now, self.rng
        )
        ...
        available_clusters = self._reachable_clusters(
            router.router_id, now, exclude={decision.cluster_id}, only_available=True
        )
        if not available_clusters:
            raise RuntimeError(
                "no clusters available to fail over after injected outage"
            )
        ...
        rerouted_request = (
            request
            if failover_delay == 0.0
            else replace(request, arrival_time=request.arrival_time + failover_delay)
        )
        retry_decision = POLICIES[policy_name](
            router, rerouted_request, available_clusters, retry_time, self.rng
        )
        return (
            rerouted_request,
            retry_decision,
    ...
    def _deliver_pending_summaries(self, now):
        while self._pending_deliveries and self._pending_deliveries[0][0] <= now:
            ...
            self.control_plane_bytes += (
                summary.byte_size + self.config.control_plane_header_bytes
            )
