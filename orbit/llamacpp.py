...
class LlamaCppClusterConfig:
    ...
    def for_cluster(self, index):
        return LlamaCppClusterConfig(model_path=self.model_path, executable=self.executable, host=self.host, port_base=self.port_base + index, threads=self.threads, ctx_size=self.ctx_size, parallel=self.parallel, request_timeout=self.request_timeout, startup_timeout=self.startup_timeout, prompt_token_cap=self.prompt_token_cap, temperature=self.temperature, top_p=self.top_p, seed=self.seed, manage_server=self.manage_server, extra_args=self.extra_args)
...
class LlamaCppClient:
    ...
    def __init__(self, base_url, request_timeout=120.0, temperature=0.0, top_p=1.0, seed=0):
    ...
    def wait_until_ready(self, timeout):
        ...
        while time.monotonic() < deadline:
            try:
                ...
                last_error = exc
                time.sleep(0.25)
        raise RuntimeError(f'llama.cpp server did not become ready: {last_error}') from last_error

    def tokenize(self, text):
        payload = self._json_request('/tokenize', payload={'content': text}, method='POST')
    ...
    def slots(self):
        ...
        for slot_payload in payload:
            ...
            slots.append(LlamaCppSlot(slot_id=int(slot_payload.get('id', -1)), context_size=int(slot_payload.get('n_ctx', 0)), is_processing=bool(slot_payload.get('is_processing', False)), task_id=int(task_id) if isinstance(task_id, int) else None))
        return tuple(slots)

    def complete(self, prompt, max_tokens, event_callback=None):
        payload = {'prompt': prompt, 'n_predict': max_tokens, 'temperature': self.temperature, 'top_p': self.top_p, 'seed': self.seed, 'stream': True, 'cache_prompt': True, 'return_progress': True}
        request_body = json.dumps(payload).encode('utf-8')
        http_request = request.Request(f'{self.base_url}/completion', data=request_body, headers={'Content-Type': 'application/json', 'Accept': 'text/event-stream'}, method='POST')
        ...
        with request.urlopen(http_request, timeout=self.request_timeout) as response:
            for raw_line in response:
                ...
                if prompt_progress is not None:
                    ...
                    prompt_eval_latency = self._extract_prompt_eval_latency(event, fallback=prompt_eval_latency)
                    if cache_ready_latency is None and prompt_progress['total'] > 0 and (prompt_progress['processed'] >= prompt_progress['total']):
        ...
        if prompt_eval_latency is None:
            prompt_eval_latency = max(0.0, min(ttft, total_latency) - processing_started_latency)
        if cache_ready_latency is None:
            cache_ready_latency = min(total_latency, processing_started_latency + max(prompt_eval_latency, 0.0))
        return LlamaCppResult(total_latency=total_latency, ttft=ttft, prompt_eval_latency=min(max(prompt_eval_latency, 0.0), total_latency), processing_started_latency=min(max(processing_started_latency, 0.0), total_latency), cache_ready_latency=min(max(cache_ready_latency, 0.0), total_latency))

    def _healthcheck(self):
        health_request = request.Request(f'{self.base_url}/health', method='GET')
        with request.urlopen(health_request, timeout=min(self.request_timeout, 5.0)) as response:
    ...
    def _json_request(self, path, payload=None, method='GET'):
        ...
        http_request = request.Request(f"{self.base_url}/{path.lstrip('/')}", data=request_body, headers=headers, method=method)
        with request.urlopen(http_request, timeout=self.request_timeout) as response:
            if response.status >= 400:
...
class ManagedLlamaCppServer:
    ...
    def start(self):
        ...
        command = [self.config.executable, '--model', str(model_path), '--host', self.config.host, '--port', str(self.config.port_base), '--threads', str(self.config.threads), '--ctx-size', str(self.config.ctx_size), '--parallel', str(self.config.parallel), '--cache-prompt', '--slots']
...
class LlamaCppCluster:
    ...
    def __init__(self, cluster_id, cluster_config=None, backend_config=None):
        ...
        self._client = LlamaCppClient(base_url=self.backend_config.base_url, request_timeout=self.backend_config.request_timeout, temperature=self.backend_config.temperature, top_p=self.backend_config.top_p, seed=self.backend_config.seed)
        self._server = ManagedLlamaCppServer(self.backend_config) if self.backend_config.manage_server else None
        self._started = False
    ...
    def _prepare_request(self, request_obj):
        ...
        if budget is not None and len(tokens) > budget:
            prompt_prefix_text = self._truncate_prompt_prefix_to_budget(prompt_prefix_text, token_budget=budget)
            tokens = self._client.tokenize(_prompt_text_from_prefix(prompt_prefix_text))
        return replace(request_obj, prompt_prefix_text=prompt_prefix_text, prefix_tokens=tokens, prefix_token_source='llama_cpp')
    ...
    def submit(self, request_obj):
        ...
        worker = threading.Thread(target=self._execute_request_async, args=(request_obj, future), daemon=True)
        worker.start()
        return future
    ...
    def publish_summary(self, now):
        with self._lock:
            self._advance_time_locked(now)
            filters = {depth: BloomFilter(num_bits=self.cluster_config.bloom_bits, num_hashes=self.cluster_config.bloom_hashes) for depth in self.cluster_config.summary_depths}
            ...
            hotsets = hot_prefix_hashes(self._cache.values(), self.cluster_config.hotset_depths, self.cluster_config.hotset_capacity_per_depth)
        ...
        return ClusterSummary(cluster_id=self.cluster_id, version=version, created_at=now, queue_depth=self.queue_depth(now), depths=self.cluster_config.summary_depths, filters=filters, byte_size=byte_size, hot_prefix_hashes=hotsets)
    ...
    def _execute_request(self, request_obj):
        ...
        def handle_event(event, elapsed):
            ...
            if prompt_progress is None:
                return
            if cache_inserted or prompt_progress['total'] <= 0 or prompt_progress['processed'] < prompt_progress['total']:
                return
            cache_ready_latency = elapsed
            with self._lock:
                self._insert_into_cache(request_obj.request_id, request_obj.prefix_tokens)
            cache_inserted = True
        try:
            completion = self._client.complete(prompt=request_obj.prompt_text, max_tokens=request_obj.continuation_tokens, event_callback=handle_event)
        finally:
            with self._lock:
        ...
        if not cache_inserted:
            ...
            with self._lock:
                ...
                heapq.heappush(self._pending_insertions, (now + completion.cache_ready_latency, self._cache_sequence, request_obj.request_id, request_obj.prefix_tokens))
                self._advance_time_locked(now + completion.total_latency)
        processing_started_latency = completion.processing_started_latency
        cache_ready_at = now + (cache_ready_latency if cache_ready_latency is not None else completion.cache_ready_latency)
        started_at = now + processing_started_latency
        finished_at = now + completion.total_latency
        return ClusterExecution(cluster_id=self.cluster_id, queue_depth_before=queue_depth_before, true_reusable_tokens=true_reusable, service_time=max(0.0, completion.total_latency - processing_started_latency), queue_delay=max(0.0, processing_started_latency), started_at=started_at, finished_at=finished_at, time_to_first_token=completion.ttft, cache_ready_at=cache_ready_at)
    ...
    def _cache_limit_exceeded(self):
        if len(self._cache) > self.cluster_config.cache_capacity:
            return True
        if self.cluster_config.cache_capacity_tokens is not None and self._cached_token_total > self.cluster_config.cache_capacity_tokens:
            return True
        return False
