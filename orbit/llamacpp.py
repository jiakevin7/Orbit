...
class LlamaCppClusterConfig:
    ...
    def for_cluster(self, index: int) -> "LlamaCppClusterConfig":
...
class LlamaCppClient:
    ...
    def slots(self) -> tuple[LlamaCppSlot, ...]:
    ...
    def complete(
        ...
        prompt: str,
        max_tokens: int,
        event_callback: Callable[[dict[str, object], float], None] | None = None,
    ) -> LlamaCppResult:
        payload = {
            "prompt": prompt,
        ...
        return LlamaCppResult(
            ...
            processing_started_latency=min(max(processing_started_latency, 0.0), total_latency),
    ...
    @staticmethod
    def _looks_like_first_token(event: dict[str, object]) -> bool:
    ...
    @staticmethod
    def _extract_prompt_eval_latency(
    ...
    @staticmethod
    def _extract_prompt_progress(event: dict[str, object]) -> dict[str, int | float | None] | None:
...
class ManagedLlamaCppServer:
    def __init__(self, config: LlamaCppClusterConfig) -> None:
        self.config = config
...
class LlamaCppCluster:
    def __init__(
        ...
        backend_config: LlamaCppClusterConfig | None = None,
    ) -> None:
    ...
    def exact_prefix_match(self, tokens: Sequence[int], now: float) -> bool:
        with self._lock:
            self._advance_time_locked(now)
            return self.trie.contains(tokens)

    def true_reusable_prefix(self, tokens: Sequence[int], now: float) -> int:
        with self._lock:
            self._advance_time_locked(now)
            return self.trie.longest_prefix(tokens)

    def execute(self, request_obj: Request) -> ClusterExecution:
        return self._execute_request(request_obj)

    def submit(self, request_obj: Request) -> Future[ClusterExecution]:
        self._ensure_started()
        future: Future[ClusterExecution] = Future()

        worker = threading.Thread(
            target=self._execute_request_async,
            args=(request_obj, future),
            daemon=True,
        )
        worker.start()
        return future

    def publish_summary(self, now: float) -> ClusterSummary:
        with self._lock:
            self._advance_time_locked(now)
            filters = {
                depth: BloomFilter(
    ...
    def _observed_busy_slots(self) -> int | None:
    ...
    def _advance_time_locked(self, now: float) -> None:
    ...
    def _execute_request_async(
        self,
        request_obj: Request,
        future: Future[ClusterExecution],
    ) -> None:
        try:
    ...
    def _execute_request(self, request_obj: Request) -> ClusterExecution:
        self._ensure_started()
        ...
        def handle_event(event: dict[str, object], elapsed: float) -> None:
        ...
        return ClusterExecution(
            ...
            service_time=max(0.0, completion.total_latency - processing_started_latency),
    ...
    def _cache_limit_exceeded(self) -> bool:
    ...
    def _add_cached_tokens(self, tokens: tuple[int, ...]) -> None:
