            ...
            if method_name == '__collect_completed__':
                ...
                for task_id, future in list(pending_tasks.items()):
                    ...
                    try:
                        ...
                        completed.append((task_id, 'error', {'type': exc.__class__.__name__, 'message': str(exc)}))
...
def _router_worker(connection, router_id, network_costs, config):
    _worker_loop(connection, lambda: Router(router_id=router_id, network_costs=network_costs, config=config))

def _llama_cluster_worker(connection, cluster_id, cluster_config, backend_config):
    _worker_loop(connection, lambda: LlamaCppCluster(cluster_id=cluster_id, cluster_config=cluster_config, backend_config=backend_config))
...
class ProcessLlamaCppClusterProxy(_ProcessProxy):
    ...
    def __init__(self, cluster_id, cluster_config, backend_config, start_method='spawn'):
        self.cluster_id = cluster_id
            ...
            with self._task_lock:
                for task_id, status, payload in completed:
                    ...
                    if status == 'ok':
                        ...
                        future.set_exception(RuntimeError(f"cluster task failed: {payload['type']}: {payload['message']}"))
